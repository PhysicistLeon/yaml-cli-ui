...
def test_root_without_launchers_fails():
    doc = _minimal_doc()
    doc.launchers = {}
    with pytest.raises(V2ValidationError, match="V2E_ROOT_LAUNCHERS_REQUIRED"):
        validate_v2_document(doc)
...
def test_pipeline_step_with_use_and_foreach_fails():
    ...
    with pytest.raises(V2ValidationError, match="V2E_STEP_MODE_EXCLUSIVE"):
        validate_v2_document(doc)
...
def test_foreach_without_as_or_steps_fails():
    ...
    with pytest.raises(V2ValidationError, match="V2E_FOREACH_AS_REQUIRED"):
        validate_v2_document(doc)
...
def test_command_without_run_fails():
    ...
    with pytest.raises(V2ValidationError, match="V2E_COMMAND_RUN_REQUIRED"):
        validate_v2_document(doc)
